...
describe('test KnifeMaker action', () => {
  ...
  it('should make action with timestamp successfully', () => {
    const { action } = (new KnifeMaker({ namespace, actionTypeTimestamp: true })).make({
      category: 'hahaha',
      actionMap: [
        'testA1',
        'testB2',
    ...
    const testcases = [
      { fn: action.testA1, typePattern: new RegExp(`${constantNamespace}@HAHAHA@TEST_A_1@[0-9]+`) },
      { fn: action.testB2, typePattern: new RegExp(`${constantNamespace}@HAHAHA@TEST_B_2@[0-9]+`) },
      { fn: action.testC3, typePattern: new RegExp(`${constantNamespace}@HAHAHA@TEST_C_3@[0-9]+`) },
    ];

    _.forEach((testcases), ({ fn, typePattern }) => {
      const { type, ...data } = fn(payload);

      data.should.to.eql(payload);
      typePattern.test(type).should.to.eql(true);
    });
  });
});
